...
**Суперадмин** в разделе «Администрирование» → «Пользователи» может создавать учётки. Поле пароля можно оставить пустым — пароль сгенерируется и уходит на email пользователя (нужны `SMTP_*` и опционально `APP_PUBLIC_BASE_URL` в `backend/.env`). После первого входа показывается ненавязчивое предложение сменить пароль в «Профиль».
